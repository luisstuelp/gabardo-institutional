...
const ScrollStack: React.FC<ScrollStackProps> = ({
  ...
  const cardInitialOffsetsRef = useRef<number[]>([]);
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    // Cache endElementTop as it's used in a loop
    const endElementTop = endElement ? getElementOffset(endElement) : 0;

    let topCardIndex = -1;
    if (blurAmount > 0) {
      for (let j = 0; j < cardsRef.current.length; j++) {
        const cardTop = cardInitialOffsetsRef.current[j];
  ...
  const setupLenis = useCallback(() => {
    ...
    if (!useWindowScroll && scroller) {
        ...
        lenisOptions.gestureOrientation = 'vertical';
    }
  ...
  useLayoutEffect(() => {
    ...
    cardInitialOffsetsRef.current = cards.map(card => getElementOffset(card));
