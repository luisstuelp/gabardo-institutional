  ...
  return (
    <section className="relative overflow-hidden bg-white py-12 sm:py-16 md:py-20">
      ...
      <div className="container mx-auto px-4 sm:px-6 md:px-8 lg:px-12">
        <div className="grid gap-8 sm:gap-10 md:gap-12 lg:grid-cols-[1.1fr_0.9fr] lg:items-center">
          <motion.div
          ...
          >
            ...
            <div className="space-y-3 sm:space-y-4">
              ...
              <p className="max-w-3xl text-sm sm:text-base md:text-lg text-neutral-600">
            ...
            <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-2">
              {capabilityPillars.map((item) => (
                <motion.div
                  ...
                  className="flex items-start gap-2 sm:gap-3 rounded-xl sm:rounded-2xl border border-gabardo-light-blue/30 bg-white/80 p-3 sm:p-4 md:p-5 shadow-sm"
                >
                  <div className="mt-1 h-1.5 w-1.5 sm:h-2 sm:w-2 flex-shrink-0 rounded-full bg-gabardo-light-blue" />
            ...
            <div className="grid gap-3 sm:gap-4">
              {infraHighlights.map((item, index) => (
                <motion.div
                  ...
                  className="group flex flex-col gap-2 sm:gap-3 rounded-2xl sm:rounded-3xl border border-white/70 bg-white/90 p-4 sm:p-5 md:p-6 shadow-[0_15px_40px_-25px_rgba(19,45,81,0.35)] transition-transform duration-500 hover:-translate-y-2 hover:shadow-[0_25px_55px_-25px_rgba(19,45,81,0.45)]"
                >
                  <MotionIconWrapper icon={item.icon} />
                  <div>
                    <h3 className="text-base sm:text-lg font-semibold text-gabardo-blue">{item.title}</h3>
                    <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm text-neutral-600">{item.description}</p>
                  </div>
                </motion.div>
              ))}
            </div>
            ...
            <div className="grid gap-3 sm:gap-4 grid-cols-1 sm:grid-cols-3">
              {metrics.map((metric) => (
                <motion.div
                  ...
                  viewport={{ once: true }}
                  transition={{ duration: 0.6, ease: 'easeOut' }}
                  className="rounded-xl sm:rounded-2xl border border-gabardo-blue/15 bg-white/90 p-4 sm:p-5 text-center shadow-sm"
                >
                  <span className="block text-xl sm:text-2xl font-bold text-gabardo-blue">{metric.label}</span>
                  <span className="mt-1.5 sm:mt-2 block text-[0.7rem] sm:text-xs text-neutral-500">{metric.text}</span>
                </motion.div>
              ))}
            </div>
          </motion.div>
