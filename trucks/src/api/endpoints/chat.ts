...
export async function sendMessage(
    ...
    // Fallback: Client-side OpenAI call if Edge Function fails/missing
    logger.warn('Edge function failed or missing, trying client-side fallback', { error: aiResult.error });

    const openAiKey = import.meta.env.VITE_OPENAI_API_KEY;
    if (!openAiKey) {
        logger.error('No OpenAI API Key found for fallback');
        return {
            data: null,
            error: { message: 'Serviço indisponível no momento.', code: 'SERVICE_UNAVAILABLE' },
            status: 503
        };
    }
...
Seja sempre cordial, profissional e objetivo. Responda em português do Brasil.`
        ...
        // 2. Call OpenAI API directly (using Vite proxy to bypass CORS)
        ...
        const aiMessageContent = data.choices?.[0]?.message?.content || "Desculpe, não entendi.";
