...
export function AnalyticsProvider({ children }: AnalyticsProviderProps) {
    useEffect(() => {
        ...
        if (gaId) {
            ...
            logger.debug('Google Analytics disabled (no VITE_GA_MEASUREMENT_ID)');
        }

        // PostHog
        const posthogKey = import.meta.env.VITE_POSTHOG_KEY;
        if (posthogKey) {
            initPostHog(posthogKey, import.meta.env.VITE_POSTHOG_HOST);
        } else {
            logger.debug('PostHog disabled (no VITE_POSTHOG_KEY)');
        }

        // Chatwoot
        const chatwootToken = import.meta.env.VITE_CHATWOOT_TOKEN;
        if (chatwootToken) {
            initChatwoot(chatwootToken, import.meta.env.VITE_CHATWOOT_BASE_URL);
        } else {
            logger.debug('Chatwoot disabled (no VITE_CHATWOOT_TOKEN)');
        }
    }, []);
